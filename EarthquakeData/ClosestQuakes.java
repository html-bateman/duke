...
public class ClosestQuakes {
    ...
    public ArrayList<QuakeEntry> getClosest(ArrayList<QuakeEntry> quakeData, Location current, int howMany) {
        ...
        for (int j = 0; j < Math.min(howMany, copy.size()); j++) {
            int minIndex = 0;
            for (int k = 1; k < copy.size(); k++) {
                QuakeEntry quake = copy.get(k);
                QuakeEntry minQuake = copy.get(minIndex);
                if (current.distanceTo(quake.getLocation()) < current.distanceTo(minQuake.getLocation())) {
                    minIndex = k;
                }
            }
